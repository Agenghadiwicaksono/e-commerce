    ...
    <section id="heroSection" class="container relative max-w-screen-xl pt-10 pb-24">
        <div class="flex flex-wrap items-center justify-between w-full gap-8">
            <div class="w-full max-w-[400px] flex flex-col gap-4">
                <div class="inline-flex gap-[6px] items-center bg-yellow-500 rounded-full px-4 py-[6px] w-max">
                ...
                <div class="mt-[14px]">
                    <a href="#eventSection" class="btn-secondary hover:[]">
                        Explore Now
                    </a>
                </div>
            </div>
    ...
    <section id="eventSection" class="container relative max-w-screen-xl py-10">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
            @foreach ($events as $event)
                ...
                :category="$event->category->name" :date="$event->start_time" :price="$event->start_from" :isPopular="$event->is_popular" :description="$event->headline" :routes="route('detail', $event->slug)"/>
            @endforeach
        </div>
    </section>
